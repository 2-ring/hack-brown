...
class ProcessingConfig:
    ...
    # Maximum events to process per request (after IDENTIFY stage)
    MAX_EVENTS_PER_REQUEST: int = EventLimits.MAX_EVENTS_PER_REQUEST

    # Max concurrent threads for STRUCTURE + PERSONALIZE pipeline per request
    MAX_WORKERS: int = int(os.getenv('DROPCAL_MAX_WORKERS', '5'))

    # Timeout per individual event processing (STRUCTURE + PERSONALIZE), in seconds
    PER_EVENT_TIMEOUT: int = 60

    # Total timeout for entire batch of events, in seconds
    BATCH_TIMEOUT: int = 300

    # --- Text Chunking (IDENTIFY stage) ---
    ...
    # Max concurrent threads for chunked IDENTIFY calls
    CHUNK_MAX_WORKERS: int = 3

    # Timeout per chunk for IDENTIFY (seconds)
